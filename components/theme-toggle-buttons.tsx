...
export const ThemeToggleButton2 = ({
  ...
  const isDark = controlledIsDark !== undefined ? controlledIsDark : localIsDark;
  const onClick = controlledOnClick || (() => setLocalIsDark(!localIsDark));
  return (
    <button
      type="button"
      className={cn(
        "rounded-full transition-all duration-300 active:scale-95 flex items-center justify-center cursor-pointer",
        className,
      )}
      onClick={onClick}
      style={{
